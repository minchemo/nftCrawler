...
connection.connect(function(err) {
    ...
    updateRecentlyNFTitem() //取得近期NFT項目，準備更新相關資料
})
...
async function processTransaction(data) {
    ...
    for (const key in grouped) {
        const nft_data = {
            nft_name: grouped[key][0]["nft_name"],
        ...
        if (nft_item_id == 0) {
            ...
            await insertTransaction(nft_item_id, grouped[key])
        }
    }
...
async function getNftItemId(item) {
    return new Promise(function(resolve, reject) {
        connection.query(
            `SELECT id, opensea_slug FROM nft_item WHERE contract_address = '${item.contract_address}' LIMIT 1`,
            function(err, result) {
                if (err) reject(err)
                resolve({
                    id: result[0].id,
                    os_slug: result[0].opensea_slug,
                })
            }
        )
    })
...
async function updateNftItemSupplys(item, supplys) {
    return new Promise(function(resolve, reject) {
        connection.query(
            `UPDATE nft_item SET supplys = ${supplys} WHERE contract_address = '${item.contract_address}'`,
            function(err) {
                if (err) reject(err)
                resolve(true)
            }
...
async function updateNftItemHolders(item, holders) {
    return new Promise(function(resolve, reject) {
        connection.query(
            `UPDATE nft_item SET holders = ${holders} WHERE contract_address = '${item.contract_address}'`,
            function(err) {
                if (err) reject(err)
                resolve(true)
            }
...
async function updateNftInfo(info, nft_data) {
    return new Promise(function(resolve, reject) {
        ...
        WHERE contract_address = '${nft_data.contract_address}'`,
...
async function getOpenseaInfo(item) {
    return new Promise(function(resolve, reject) {
        ...
        fetch(
                `https://api.opensea.io/api/v1/asset_contract/${item.contract_address}`,
                options
            )
...
async function insertTransaction(nft_item_id, list) {
    return new Promise(function(resolve, reject) {
        ...
        let sql =
            "INSERT IGNORE INTO mint_transactions (nft_item_id, txn_hash, token_id, timestamp) VALUES ?"
        ...
        request({
            ...
            (error, res, body) => {
                ...
                for (let i = 1; i < list.length; i++) {
                    ...
                    if (!from.match("Null Address")) {
                        //非鑄造
                        continue
                    } else if (parseInt(tokenId) > 50000) {
                        //token數量過多，暫時排除
                        continue
                    } else {
...
/**
 * 更新近期NFT項目資訊 (supplys, holders, OSdata)
 * @returns
 */
async function updateRecentlyNFTitem() {
    let items = await getRecentlyNFTitem()

    console.log("\x1b[36m%s\x1b[0m", items.length + " NFT項目等待更新")

    for (let i = 0; i < items.length; i++) {
        console.log("\x1b[36m%s\x1b[0m", `正在更新第 ${i + 1} 個項目`)
        ...
        let supplys = await getTokenSupplys(element.contract_address)
        if (supplys != 0 && supplys) {
            await updateNftItemSupplys(nft_data, supplys)
        }

        let holders = await getTokenHolders(element.contract_address)
    ...
    console.log(
        "\x1b[36m%s\x1b[0m",
        `已更新 ${items.length} 個項目，準備下一次更新`
    )

    updateRecentlyNFTitem()
}
 ...
 * @returns
 */
async function getRecentlyNFTitem() {
    return new Promise(function(resolve, reject) {
        let time = Math.floor(Date.now() / 1000)
        time = time - 600
        connection.query(
            `
                SELECT COUNT(*) as count, nft_item.contract_address
                FROM mint_transactions 
                LEFT JOIN nft_item
                ON mint_transactions.nft_item_id = nft_item.id
                WHERE mint_transactions.timestamp > ${time}
                GROUP BY nft_item.contract_address
                HAVING count > 2
                `,
            function(err, res) {
                if (err) reject(err)
